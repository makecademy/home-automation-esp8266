...
void loop() {
  ...
  s += "<script>$.ajax({url: \"http://api.openweathermap.org/data/2.5/weather?q=lublin,poland\", crossDomain: true})";
  s += ".done(function(result) {$(\"#ext_temp\").html((result.main.temp -  273.15).toFixed(2)); $(\"#ext_humidity\").html(result.main.humidity);})";
  s += "</script>";
